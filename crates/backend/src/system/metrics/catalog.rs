...
pub static METRIC_CATALOG: &[MetricDef] = &[
    // --- Знания ------------------------------------------------------------
    // Две, а не девять: показывать стоит только то, у чего есть направление.
    // Остальное живёт на странице «Инвентаризация знаний». Храповик их не
    // гейтит (он сравнивает codebase_metrics.json, а это рантайм) — и правильно:
    // orphan_tools зависит от навыков экземпляра. Разбор — knowledge/llm.md.
    with_limits(
        def(
            "knowledge.unreachable_surfaces",
            "Поверхностей знания без инструмента",
            "knowledge",
            "шт",
            0,
            Lower,
    ...
    with_limits(
        def(
            "knowledge.orphan_tools",
            "Инструментов вне ядра и навыков",
            "knowledge",
            "шт",
            0,
            Lower,
    ...
    with_hint(
        ...
        "Прямая мера сложности: такой файл уже не читается целиком. Считаются только рукописные файлы — *_gen.rs исключены, их не разбить",
    ),
    with_hint(
        def(
            "code.top10_share",
    ...
    with_hint(
        ...
        "Порог, за которым файл перестаёт быть большим и становится хабом. Как и счётчик выше, без *_gen.rs",
    ),
    with_hint(
        with_limits(
    ...
    // Отдельный счётчик, а не часть arch.naming_violations: это единственная
    // метрика, по которой видно, далеко ли до разреза на крейты. Считает и
    // закрытые waiver'ом места — «нарушений нет» и «работы не осталось» здесь
    // разные утверждения, и второе важнее.
    with_hint(
        with_limits(
            def(
                "arch.core_backedges",
                "Ядро знает маркетплейсы",
                "domain",
                "файлов",
                0,
                Lower,
        ...
        "Файлы ядра, называющие маркетплейсные модули по имени. Пока их больше нуля, вынести маркетплейсы в отдельный крейт нельзя — получится цикл (docs/plans/2026-08-29-crate-split-analysis.md)",
    ),
    // --- Границы и контракты -----------------------------------------------

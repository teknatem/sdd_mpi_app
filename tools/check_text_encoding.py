...
# Историческая кириллица (U+0460-U+047F) — та же мысль, что и у SUSPECT, но
# целым блоком: в современном русском её нет, значит "расшифровка" в неё
# означает случайное совпадение, а не порчу.
#
# Без этого текст «Подключения 1С» ловился как порча: пара U+0421 U+00BB
# (заглавная Эс + закрывающая кавычка) разбирается в одинокую U+047B, проверка
# длины на границе 2->1 её пропускала, и --fix ИСПОРТИЛ БЫ корректный текст.
# Настоящие пары вида U+0420 U+00BB по-прежнему чинятся: их результат лежит в
# основном блоке, а не в историческом.
#
# Примеры здесь записаны кодовыми точками намеренно: файл проверяет сам себя,
# и литеральный mojibake в комментарии неотличим от настоящей порчи.
ARCHAIC = re.compile("[" + chr(0x460) + "-" + chr(0x47F) + "]")
    ...
    for _ in range(4):
        ...
        if not MANGLED.search(current):
            clean = (
                ...
                and not ARCHAIC.search(current)

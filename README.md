...
[SDD.md](../sdd/SDD.md); правила именно этого приложения — в `architecture.toml`.

## Место в экосистеме

MPI — приложение экосистемы **SDD**, живущей в `F:\dev`. Соседи: `sdd_studio`
(анализатор структуры), `sdd_desktop` (тонкий клиент), `sdd` (канон: имена, метод,
схема манифеста, контракты).
...
забирает число findings. Границы — [CONTRACTS.md](../sdd/CONTRACTS.md), карта —
[ECOSYSTEM.md](../sdd/ECOSYSTEM.md). В `memory-bank` и исходники соседних репозиториев
не заглядываем.

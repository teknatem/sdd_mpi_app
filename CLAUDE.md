...
> **Что из кода не выводится — генерируется из данных**: профиль таблиц (строк, период,
> заполненность ссылок), плагины из БД, навыки и quality-checks из рантайма. Живёт в
> `<data>/knowledge/generated/` со штампом `kind: generated`, обновляется при старте и по
> `POST /api/kb/generate`. Эти карты **не попадают** в обычную выдачу `search_knowledge`
> (иначе утопили бы курируемые бизнес-статьи) — их достают явным `corpus="generated"`,
> по id или через якоря. Профиль таблицы дополнительно приезжает в `get_entity_schema`.
>
> Каталоги `app/` (копия техдокументации из бинарника) и `generated/` внутри базы знаний
> принадлежат приложению: корпус документа определяется каталогом, запись туда запрещена,
> лишние файлы вычищаются при загрузке, в наборы данных они не переносятся.
> **Якорь** — привязка статьи к объекту (`entities: [a012]` или код в `related`): даёт
> `search_knowledge(entities=[...])` и поле `docs` в `get_entity_schema`.

...
> крейта — **26 с**. Конвейер `trunk` добавляет к фронту постоянные **~11 с**
> `wasm-bindgen` даже когда исходники не менялись, так что реальный цикл
> фронта ≈ **30 с** — самый дорогой в проекте.
>
